...
using PokemonSwitch.Droid.Extensions;
using PokemonSwitch;
        ...
        private async Task SetImageSourceAsync(Android.Widget.Button targetButton, ImageButton model)
        {
            ...
            using (var bitmap = await GetBitmapAsync(source).ConfigureAwait(false))
            {
                ...
                else
                {
                    ...
                    using (var scaledDrawable = GetScaleDrawable(drawable, GetWidth((int)model.WidthRequest), GetHeight((int)model.HeightRequest)))
                    {
                        Drawable left = null;
                        Drawable right = null;
                        Drawable top = null;
